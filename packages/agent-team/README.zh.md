...
Team 自己就附带一个这样的 producer：`wowyuarm-agent-team-routines` 行（`@wowyuarm/dsh-agent-team/routines`），其 `config.routines` 列出要触发什么。每条 entry 都声明且只声明一个 trigger——一个五字段 cron 表达式，`分 时 日 月 周`，按 Host 自己的时钟读取，所以 `0 9 * * 1` 是 Host 所在时的每周一 09:00——并且只做一件事：点名一个 Member（handle 或带品牌的 `member:<uuid>` id），携带要注入该 Member 自己 Session 的指令。routine 不能替任何人说话——它不提交 Message、也不开 Thread，所以一次触发启动的是该 Member 的 turn，且只有已激活并持有 live session 的 Member 才能被唤醒。`once: true` 在下一次出现时刻触发一次，之后永不再触发。无法运行的声明会在该行挂载时报错，因为「静默地永不触发」正是无人值守的 producer 事后无法报告的失败；五年内一次都不会到的表达式会带名字被拒。

被触发的 wake 以本行自己的 source `kind` 作为 `notice` 抵达 Member，标题为 `[ROUTINE FIRE] <name> — fired <instant> (cron '<expression>')`，带上 Team 固定的 UTC+8 时刻，并声明这个 turn 无人值守、该对话里没有人在等回复。每次触发都追加到 `$DSH_HOME/agent-team/routines/fires.jsonl`：`delivered` 带投递通道、Session id，以及这条 routine 是否为一次性；`failed` 带 `reason`（`unknown-member`、`member-not-enabled`、`no-live-session`、`wake-failed`）与其 message；`not-armed` 留给「没有未来的出现时刻」的声明。该记录是尽力而为的——日志不可写时只 warn，不会变成第二个失败——超过 256 KiB 后只保留最新 200 行，一次性的 `delivered` 行除外：正是它让 `once` routine 在重启后保持已用尽。bundle 以未配置状态插入这一行，因为 routine 指名的 Member 与表达式不是 bundle 能知道的；操作者通过在自己 profile 里 patch 该行的 id 来提供 schedule，而 patch 会替换该行的整个 `config`。

从 Team GUI 或由 `team_routine` 工具保存的 routine 不能放进那一行：profile 的 YAML 属于操作者，实时编辑不应改写它。这些 routine 与 fire log 放在一起：`$DSH_HOME/agent-team/routines/routines.json`，并经与 `config.routines` 相同的校验器读取。该文件一旦变化，这一行就重新读取并重新 arm 整个 schedule，因此在 GUI 中或通过工具新增、修改或删除 routine 无需重载 profile、也无需重启 Host 即可生效。store 拥有它声明的任何名字：`config.routines` 中的同名 entry 会被遮蔽而不是重复，所以一个名字始终只 arm 一个 routine、只触发一次。不可用的 store 不会让 schedule 付出代价——只 warn 一次，config 声明的 routine 照常运行；写入先校验后落地、以 rename 落地，因此被拒绝的编辑会保留原文件。
