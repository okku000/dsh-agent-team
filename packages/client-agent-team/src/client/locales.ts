export const zh = {
  ...
  routinesTitle: '定时任务',
  routinesCount: '{count} 条定时任务',
  routinesHint: '定时任务只唤醒一个 Agent，不会往频道里发消息',
  routinesLoading: '正在加载定时任务…',
  routinesEmptyTitle: '还没有定时任务',
  routinesEmptyHint: '新建一条，让某个 Agent 在指定时间自己去做一件事',
  routinesNoWorkspace: '还没有 Workspace，无法读取排程',
  routinesNew: '新建定时任务',
  routinesEdit: '编辑',
  routinesDelete: '删除',
  routinesDeleting: '正在删除…',
  routinesDeleteTitle: '删除「{name}」？',
  routinesDeleteNotice: '删除后这条定时任务不会再触发',
  routinesDeleteHint: '已在运行的排程会立刻更新；profile 里声明的条目不能在这里删除。',
  routinesCreateTitle: '新建定时任务',
  routinesEditTitle: '编辑定时任务',
  routinesCreateNotice: '定时任务到点会唤醒一个 Agent Member，由它自己决定怎么做',
  routinesSave: '保存',
  routinesSaving: '正在保存…',
  routinesName: '名称',
  routinesNamePlaceholder: '例如 morning-check',
  routinesMember: '唤醒谁',
  routinesWakes: '唤醒 {member}',
  routinesPrompt: '指令',
  routinesPromptPlaceholder: '例如：检查一下模型目录有没有变化，有变化就记下来',
  routinesSummary: '摘要（可选）',
  routinesSummaryPlaceholder: '显示在唤醒通知上的一行说明',
  routinesCron: 'cron 表达式',
  routinesCronPlaceholder: '*/15 * * * *',
  routinesCronHint: '五个字段：分 时 日 月 周。例如 0 9 * * 1 是每周一 09:00，*/15 9-17 * * mon-fri 是工作日每 15 分钟。',
  routinesZone: '按 Host 所在时区（{zone}）计算',
  routinesNextFires: '接下来 3 次',
  routinesNextFire: '下次 {at}',
  routinesStopAfterFirst: '触发一次后停止',
  routinesOriginConfig: 'profile 声明',
  routinesOriginConfigMeta: '由操作者在 profile 中声明',
  routinesCreatedBy: '{name} 创建于 {time}',
  routinesUpdatedBy: '{name} 更新于 {time}',
  routinesOnceSummary: '只触发一次',
  routinesRepeatSummary: '每次到点都触发',
  routinesErrorName: '名称只能以字母或数字开头，只含字母、数字、- 和 _',
  routinesErrorMember: '请选择要唤醒的 Agent',
  routinesErrorPrompt: '请填写指令',
  routinesErrorCron: '请填写 cron 表达式',
  routinesCronInvalid: '这个 cron 表达式读不出来：{message}',
  routinesCronNeverFires: '这个表达式在五年内不会触发，请检查日、月、周字段',
  channelName: '名称',
...
export const en = {
  ...
  routinesCount: '{count} scheduled',
  routinesHint: 'A routine wakes one Agent Member and never posts into a Channel',
  routinesLoading: 'Loading routines…',
  routinesEmptyTitle: 'Nothing is scheduled',
  routinesEmptyHint: 'Create one to have an Agent do something on its own at a set time',
  routinesNoWorkspace: 'No Workspace yet, so the schedule cannot be read',
  routinesNew: 'New routine',
  routinesEdit: 'Edit',
  routinesDelete: 'Delete',
  routinesDeleting: 'Deleting…',
  routinesDeleteTitle: 'Delete "{name}"?',
  routinesDeleteNotice: 'A deleted routine will not fire again',
  routinesDeleteHint: 'The running schedule updates at once; an entry declared in the profile cannot be deleted here.',
  routinesCreateTitle: 'New routine',
  routinesEditTitle: 'Edit routine',
  routinesCreateNotice: 'At its instant a routine wakes one Agent Member, which decides what to do about it',
  routinesSave: 'Save',
  routinesSaving: 'Saving…',
  routinesName: 'Name',
  routinesNamePlaceholder: 'e.g. morning-check',
  routinesMember: 'Wake',
  routinesWakes: 'Wakes {member}',
  routinesPrompt: 'Instruction',
  routinesPromptPlaceholder: 'e.g. check the model catalog for changes and note them',
  routinesSummary: 'Summary (optional)',
  routinesSummaryPlaceholder: 'One line shown on the wake notice',
  routinesCron: 'Cron expression',
  routinesCronPlaceholder: '*/15 * * * *',
  routinesCronHint: 'Five fields: minute hour day-of-month month day-of-week. For example 0 9 * * 1 is every Monday at 09:00, and */15 9-17 * * mon-fri is every 15 minutes of the working day.',
  routinesZone: "Read on the Host's own clock ({zone})",
  routinesNextFires: 'Next 3 fires',
  routinesNextFire: 'next {at}',
  routinesStopAfterFirst: 'Stop after the first fire',
  routinesOriginConfig: 'declared in profile',
  routinesOriginConfigMeta: 'Declared by the operator on the profile row',
  routinesCreatedBy: '{name} created {time}',
  routinesUpdatedBy: '{name} updated {time}',
  routinesOnceSummary: 'fires once',
  routinesRepeatSummary: 'repeats',
  routinesErrorName: 'The name must start with a letter or digit and hold only letters, digits, - and _',
  routinesErrorMember: 'Choose the Agent to wake',
  routinesErrorPrompt: 'An instruction is required',
  routinesErrorCron: 'A cron expression is required',
  routinesCronInvalid: 'This cron expression cannot be read: {message}',
  routinesCronNeverFires: 'This expression never fires within five years; check the day-of-month, month and day-of-week fields',
  channelName: 'Name',
