...
A row answers, in reading order, what the routine is, when it fires, who it wakes, and who asked for it. The trigger repeats the expression the declaration carries, because that string is what a reader edits, and adds the next fire the Host's own parser gives for it. The wake target is drawn as `@handle` resolved from the roster, falling back to the declaration's own text when the roster no longer carries that Member, because that target is what the Host will fire at; and the attribution names who saved the routine and when.
...
The editor is one dialog: the name, the Member to wake, the instruction, an optional summary, and exactly one trigger — a five-field cron expression read on the Host's own clock, with `once` to stop after the first fire. The name is the identity, so creating asks for it while editing keeps it fixed.

The expression field reads back what it means rather than only accepting input: it carries the grammar, the zone the Host will read it in, and the next three fires that expression would keep, so a reader checks the schedule instead of forming a second opinion about it. The Client reads expressions with the Host's own parser for exactly that reason, so what the field accepts is what the Host accepts.

The form validates what the Host validates — a name that cannot be a log line, a missing Member or instruction, an expression that cannot be read, a day rule that never comes round — and names the offending field before the save is attempted. A declaration the Host refuses comes back as the Host's own reason, with the dialog still open and the running schedule untouched.
