...
一行按阅读顺序回答：这条任务是什么、何时触发、唤醒谁、谁要求的。触发方式原样复述声明里携带的那个表达式——读者编辑的就是这个字符串，所以行上画的也必须是它——随后补上 Host 自己的解析器算出的下一次触发。唤醒目标画成经名册解析出的 `@handle`，名册里已经没有这个 Member 时回落到声明原文——因为那才是 Host 真正会触发的东西；归属行写明是谁、何时保存的。
...
编辑器是一个弹层：名称、要唤醒的 Member、指令、可选摘要，以及恰好一种触发方式——一个按 Host 自己时钟读取的五字段 cron 表达式，`once` 表示首次触发后停止。名称即身份：新建时询问，编辑时固定不变。

表达式字段不只接受输入，还回读它的含义：字段下带语法提示、Host 读取它的时区，以及该表达式接下来 3 次触发，读者因此是在核对排程，而不是形成第二份理解。Client 用 Host 自己的解析器读表达式，正是为了这一点：字段接受的就是 Host 接受的。

表单先校验 Host 会校验的东西——不能成为日志行的名称、缺 Member 或指令、缺失或读不出的表达式、不会到来的日规则——并在发起保存前点名出错字段。Host 拒绝的声明会带着 Host 自己的原因回来，弹层保持打开，正在运行的排程不受影响。
