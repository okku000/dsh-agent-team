...
- You can now see and change everything the Team is scheduled to do on its own from one place: a **Routines** entry stands beside the Inbox in the sidebar and lists every routine with the cron expression it fires on, the next fire that expression produces, the Agent Member it wakes, the instruction it injects, and who created it. Creating, editing, and deleting one there needs no profile edit and no Host restart; a routine the operator declared in the profile is marked as such and is not editable from the page; and a save the Host refuses is answered with the Host's own reason in the dialog. The page shows the schedule rather than the outcome of a fire, because a routine save is configuration rather than a Team fact — a fire's result still lives only in the Host's fire log.
- The Team can now start work on its own: a routine fires a scheduled instruction into one named Agent Member's own session, so a Member can be woken to check something on a recurring schedule without anyone typing a message. A routine declares one five-field cron expression, read on the Host's own clock, and the listing names that zone so a schedule written in another one is not misread; `once: true` fires at the next occurrence and then never again. An expression the Host cannot read — an `@daily`-style macro, a six-field form, or one that never comes round within five years — is refused with the reason when it is saved rather than left to fire never. Routines are configured on the Team's own `wowyuarm-agent-team-routines` row, and every fire is recorded under `$DSH_HOME/agent-team/routines/fires.jsonl` — delivered with its lane and session, or refused with the reason.
...
- Any Agent Member can now schedule that same work itself: the new `team_routine` tool lists what is scheduled, saves or replaces one routine by name, and deletes it, so a routine a Human asks for in conversation lands in the same store the GUI writes. Each stored routine records who saved it and when, the running Host arms a save without a restart, a declaration it cannot run is refused whole with the reason, and a routine the operator declared in the profile is listed but cannot be saved over or deleted from the tool.
...
- Long messages read as documents: a body past the fold takes a roomier rhythm of paragraph, list and line spacing, short chat keeps its dense grid, and message text follows the content-size setting.
- The Team Client takes DSH 0.1.7's surface language: sidebar rows, the Workspace selector and member rows share one radius tier, the composer card wears DSH's own hairline and elevation, a narrow composer narrows its gaps instead of dropping the as-task word, and the Inbox row's clock yields when the line cannot hold it.
- Fixed the `@mention` menu being hard to read: DSH 0.1.7 made the shared menu surface translucent, and the suggestion list now frosts what is behind it instead of letting the conversation bleed through the names.
